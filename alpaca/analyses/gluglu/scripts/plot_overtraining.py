...
def make_plot(input_file, tname, histos_sel, variables, bins, labels, colors, mass, name_selection, all_same_sel=False, unit_area=False, overflow=True, output_folder='./'):
    myHistos = [myHisto(input_file[i], tname, histos_sel[i], variables[i], bins, labels[i], colors[i], mass+' [GeV]', overflow=overflow) for i in range(len(variables))]
    ...
    c.SaveAs(output_folder+'/overtrain_'+name_can+'.pdf')


def options():
    parser = argparse.ArgumentParser(usage=__doc__, formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--file-train', required=True, help="Input ROOT file")
    parser.add_argument('--file-test', required=True, help="Input ROOT file")
...
def main():
    ...
    for mass in ['mt1','mt2']:
        ...
        variables = [mass+'_reco', mass+'_reco']
        labels = ['train','test']
        ...
        for isel,sel in enumerate(selections):
            ...
            for overflow in [True,False]: 
                for unit_area in [True,False]:
                    make_plot(fnames, tname, histos_sel, variables, bins, labels, colors, mass, name_selections[isel], all_same_sel=True, unit_area=unit_area, overflow=overflow, output_folder=args.output_folder)
