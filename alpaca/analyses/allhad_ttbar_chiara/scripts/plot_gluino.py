...
def make_plot(input_file, tname, histos_sel, variables, bins, labels, colors, mass, name_selection, all_same_sel=False, unit_area=False, overflow=True, output_folder='./'):
    myHistos = [myHisto(input_file, tname, histos_sel[i], variables[i], bins, labels[i], colors[i], mass.replace('t','g')+' [GeV]', overflow=overflow) for i in range(len(variables))]
    ...
    for ih,h in enumerate(histos): 
        ...
        h.Draw("hist same")
    #legend
    leg=ROOT.TLegend(0.6, 0.7, 0.875, 0.87)
    leg.SetFillStyle(0)
    leg.SetLineColor(0)
    leg.SetLineWidth(0)
...
def main():
    ...
    for mass in ['mt1', 'mt2']:
        ...
        for isel,sel in enumerate(selections):
            ...
            add_label = ['true'] if 'truth' in sel else []
            print(sel)
