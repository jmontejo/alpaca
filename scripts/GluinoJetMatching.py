...
print("Making pandas")
nom_iter = uproot.pandas.iterate(fname,"trees_SRRPV_",entrysteps=float("inf"),branches=["eventNumber","jet_*"])
truth_iter = uproot.pandas.iterate(fname,"trees_SRRPV_",entrysteps=float("inf"),branches=["eventNumber","truth_QuarkFromGluino_phi",'truth_QuarkFromGluino_eta'])#, 'truth_QuarkFromGluino_ParentBarcode'])
...
for nom_df,truth_df  in zip(nom_iter,truth_iter):
    ...
    col_new = [c+'_'+v for v in ['phi','eta'] for c in partons]
    print(truth_df.head())
    print(truth_df.columns)
    print(col_new)
    truth_df.columns = col_new
